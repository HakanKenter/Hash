<?php 

define("ERR_BADID", "Nom d'utilisateur ou mot de passe incorrect.");
define("ERR_MISSING", "Veuillez remplir tous les champs.");
define("ERR_USERNAME", "Nom d'utilisateur trop court.");
define("ERR_PASSWORD", "Mot de passe trop faible.");
define("ERR_CONFIRM", "Les mots de passe ne correspondent pas.");
define("ERR_INTERNAL", "Problème interne.");
define("ERR_AEXIST", "Nom d'utilisateur déjà existant.");
define("ERR_historique", "Une erreur dans l'envoi du message.");
define("ERR_ADD", "Erreur dans l'ajout de image.");